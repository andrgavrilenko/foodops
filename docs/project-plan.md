...
| AI/ML            | OpenAI GPT-4o-mini (Plan B: DeepSeek V3), Handlebars (промпт-шаблоны)                  |
...
| 2.4  | Интеграция с OpenAI Structured Outputs | Вызов GPT-4o-mini с JSON-схемой. Обработка streaming (SSE для фронтенда). Retry-логика (до 3 попыток при невалидном ответе).                                                                                                           | 2.1, 2.3    | P0        | 2 дн         | AI-engineer |
...
| M3  | AI генерирует меню                          | GPT-4o-mini генерирует валидное недельное меню по профилю семьи. Валидация проходит в >80% случаев.              | W7             | Sprint 3-4 | POST /menu/generate возвращает валидное меню за <15 сек                    |
...
| AI (OpenAI)    | Pay-as-you-go (GPT-4o-mini)     | $20-50         |
