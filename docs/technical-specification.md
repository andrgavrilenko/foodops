...
- Начальный магазин: S-Market (платформа S-kaupat.fi).
...
| US-031 | Как пользователь, я хочу нажать кнопку "Добавить в корзину" в расширении, чтобы все товары из моего списка автоматически добавились в корзину на S-kaupat.fi. | Высокий   |
...
| ID     | Требование                          | Описание                                                                                                                                                                                                                                                                 |
| ------ | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| FR-310 | Навигация на S-kaupat.fi            | При нажатии кнопки "Добавить в корзину" расширение открывает/переключается на S-kaupat.fi (если не открыт).                                                                                                                                                              |
| FR-311 | Проверка авторизации на S-kaupat.fi | Расширение проверяет, авторизован ли пользователь на S-kaupat.fi. Если нет -- уведомляет пользователя о необходимости войти в аккаунт S-Market.                                                                                                                          |
| FR-312 | Последовательное добавление товаров | Расширение последовательно ищет каждый товар из списка на S-kaupat.fi и добавляет его в корзину.                                                                                                                                                                         |
| FR-313 | Поиск товара                        | Для каждой позиции расширение выполняет поиск по названию товара (на финском) на S-kaupat.fi.                                                                                                                                                                            |
...
| FR-322 | Ручной выбор      | Для не найденных товаров пользователь может перейти на S-kaupat.fi и добавить товар вручную.                                                            |
...
| NFR-003 | Время добавления одного товара в корзину | < 10 секунд на один товар (включая поиск и добавление на S-kaupat.fi). |
...
| NFR-024 | Chrome Extension безопасность        | Расширение запрашивает минимально необходимые permissions. Не хранит токены S-kaupat.fi. Коммуникация только через background script. Content script имеет доступ только к домену S-kaupat.fi. |
...
| NFR-032 | Graceful degradation  | При недоступности AI-сервиса система предлагает меню из предварительно сгенерированных шаблонов. При недоступности S-kaupat.fi расширение уведомляет пользователя и предлагает экспорт списка в текстовый формат. |
...
|                                                              |
|  +---------------------+  +-----------------------------+    |
|  | OpenAI API /        |  | S-Market (S-kaupat.fi)        |    |
...
| **Content Script**            | Внедряется на S-kaupat.fi. Выполняет поиск товаров, добавление в корзину, чтение результатов поиска. Манипулирует DOM страницы S-kaupat.fi. |
...
```json
{
  ...
  "host_permissions": ["https://www.S-kaupat.fi/*", "https://api.foodops.app/*"],
  "content_scripts": [
    {
...
3. Background SW проверяет наличие открытой вкладки S-kaupat.fi
...
| foodie_product_id | VARCHAR(100) | Идентификатор на S-kaupat.fi      |
...
| 2         | Foodie.fi Open Data     | Проверить наличие открытых данных или партнёрской программы на S-kaupat.fi.                                                                                                   |
| 3         | Web Scraping            | Парсинг каталога с S-kaupat.fi. Требуется: соблюдение robots.txt, ограничение частоты запросов, обработка изменений структуры страниц. Юридические риски -- требуется оценка. |
...
#### 8.1.2 Chrome Extension и S-kaupat.fi

Chrome Extension взаимодействует с S-kaupat.fi через Content Script, который манипулирует DOM страницы.

**Ключевые взаимодействия:**

| Действие             | Механизм                                                       |
| -------------------- | -------------------------------------------------------------- |
| Поиск товара         | Ввод текста в поле поиска на S-kaupat.fi, ожидание результатов |
...
- Изменение структуры DOM S-kaupat.fi потребует обновления Content Script.
...
| --------------- | ----------------- | ------------------------------------------------------------------------------ | ------------------------------------------ |
| **OpenAI**      | GPT-4o-mini       | Отличное соотношение цена/качество, Structured Outputs, достаточно для меню    | Зависимость от внешнего сервиса            |
| **DeepSeek**    | DeepSeek V3       | Низкая стоимость, хорошее качество, Plan B при проблемах с OpenAI              | Менее зрелый API                           |
| **OpenAI**      | GPT-4o            | Высокое качество генерации, хорошее следование инструкциям                     | В 10x дороже GPT-4o-mini                   |
| **Open-source** | Llama 3 / Mixtral | Нет зависимости от внешних API, контроль данных                                | Требует инфраструктуры, ниже качество      |

**Решение для MVP:** OpenAI GPT-4o-mini с Structured Outputs. Plan B — DeepSeek V3. Абстрагировать LLM-провайдер через интерфейс для возможности переключения.
...
| Стоимость за запрос (GPT-4o-mini)               | ~$0.003                  |
| Среднее число запросов на пользователя в неделю | 2-3 (генерация + замены) |
| Стоимость на 1000 пользователей в месяц         | ~$36-54                  |
...
- [x] Автоматическое добавление товаров в корзину на S-kaupat.fi
...
| Критерий              | Описание                                                                                      |
| --------------------- | --------------------------------------------------------------------------------------------- |
| Полный цикл           | Пользователь может пройти путь от регистрации до добавления товаров в корзину на S-kaupat.fi. |
...
| Успешность добавления | > 90% товаров из списка успешно добавляются в корзину на S-kaupat.fi.                         |
...
| Content Script | **TypeScript (vanilla)**                       | Минимальный бандл для инъекции на S-kaupat.fi |
| Тестирование   | **Puppeteer** / **Playwright**                 | E2E тесты взаимодействия с S-kaupat.fi        |
...
| Framework      | **Fastify**                          | Высокая производительность, низкий overhead, простота, активная экосистема плагинов           |
...
| Компонент         | Технология                                      | Обоснование                             |
| ----------------- | ----------------------------------------------- | --------------------------------------- |
| LLM-провайдер     | **OpenAI API** (GPT-4o-mini), Plan B: DeepSeek  | Structured Outputs, низкая стоимость    |
...
| ---------------------------------------------------------------------------------------------------------------------------------- | ----------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Изменение DOM S-kaupat.fi** -- структура страниц S-kaupat.fi может измениться без предупреждения, сломав Content Script          | Высокая     | Критическое | Абстрагировать DOM-селекторы в конфигурационный файл. Мониторинг работоспособности расширения. Автоматические тесты против S-kaupat.fi. Быстрый release pipeline для обновления расширения. |
| **Блокировка Chrome Extension** -- S-kaupat.fi может заблокировать автоматизированные действия (rate limiting, CAPTCHA, block)     | Средняя     | Критическое | Добавить реалистичные задержки между действиями (2-5 сек). Не превышать разумное количество действий за сессию. Исследовать партнёрство с S Group.                                          |
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Парсинг S-kaupat.fi**          | Web scraping каталога может нарушать Terms of Service.                                                 | Приоритет -- получение официального API доступа. Если парсинг -- соблюдать robots.txt, минимальная нагрузка. Юридическая консультация.                         |
| **Автоматизация на S-kaupat.fi** | Автоматическое добавление товаров в корзину может нарушать ToS.                                        | Расширение работает от лица пользователя (его аккаунт, его действия). Изучить ToS S-kaupat.fi. Рассмотреть партнёрство.                                        |
...
| Ресурс                                    | URL                                                        |
| ----------------------------------------- | ---------------------------------------------------------- |
| S-kaupat.fi                               | https://www.S-kaupat.fi                                    |
